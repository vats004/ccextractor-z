...
//
// if (hi is between 0x01 and 0x0F) // normal start codes
// {
//     calculate xds_class from hi
//     calculate is_new from hi
//
//     look through all buffers:
//         if (find matching buffer)
//             use it
//         else if (find free buffer)
//             use it
//         else
//             all full => give up
//
//     if (starting new packet OR buffer wasn't in use)
//         reset the buffer (empty it)
//
//     if (not new)
//         return; // do not add data yet
// }
// else
// {
//     // informational packet or illegal data
//     if (hi or lo are illegal)
//         log "illegal data" and return
// }
//
// // finally, store hi and lo into the buffer
...
// function xds_do_copy_generation_management_system(sub, ctx, c1, c2):
//     if ctx is missing:
//         return
//     extract bit c1_6 from c1
//     extract bit c2_6 from c2
//     if c1_6 is not set or c2_6 is not set:
//         return
//
//     if (c1, c2) changed since last time:
//         decode:
//             - CGMS (copy permission)
//             - APS (analog protection)
//             - RCD (redistribution control)
//         format them into strings
//         save c1 and c2 as last_c1 and last_c2
//
//     always:
//         - print copy permission string
//         - print APS string
//         - print RCD string
//     if changed:
//         - log these strings
...
// function handle_content_advisory(subtitle, context, c1, c2):
//     if context is missing:
//         return
//     if certain important bits (bit 6) of c1 or c2 are not set:
//         return
//
//     if c1 or c2 has changed since last time:
//         mark as 'changed'
//         save current c1, c2 as 'last seen'
//
//         decode important bits from c1 and c2:
//             (figure out what kind of content advisory it is)
//
//         if it's US TV guidelines:
//             make a string like "TV-14", "TV-Y7"
//             list if there's violence, sexual content, etc.
//
//         if it's a movie rating (MPA):
//             make a string like "PG", "R", "G"
//
//         if it's Canadian English or French rating:
//             make a string like "Viewers 14+", "Adult Programming"
//
//     if it's US TV guidelines:
//         print the age rating
//         print the content warnings (violence, sexual situations)
//
//     if it's MPA or Canadian rating:
//         print the rating
//
//     if it changed but decoding wasn't supported:
//         log that it's "unsupported content advisory"
...
// function xds_do_current_and_future(subtitle, context):
//     if context is null:
//         return error
//
//     allocate string buffer
//     initialize was_processed flag to false
//
//     switch (current xds packet type):
//         case START_TIME:
//             extract minutes, hours, date, month from payload
//             if values changed from previous:
//                 update context and reset shown flag
//             log and print start time info
//             if not shown yet and is current program:
//                 log program change and update GUI
//
//         case LENGTH_AND_CURRENT_TIME:
//             extract program length (hours, minutes)
//             optionally extract elapsed time and seconds
//             log and print program length info
//             set shown flag
//
//         case PROGRAM_NAME:
//             extract program name from payload
//             log and print program name
//             if name changed and is current program:
//                 log program change and update GUI
//
//         case PROGRAM_TYPE:
//             extract program type codes from payload
//             decode program types using lookup table
//             log and print program types
//             set reported flag
//
//         case CONTENT_ADVISORY:
//             call content advisory handler
//
//         case AUDIO_SERVICES:
//             mark as processed (no action)
//
//         case CGMS:
//             call copy generation management system handler
//
//         case ASPECT_RATIO_INFO:
//             calculate aspect ratio info from start/end lines
//             log and print aspect ratio details
//
//         case PROGRAM_DESCRIPTION (1-8):
//             extract description line from payload
//             if description changed:
//                 log and update GUI
//             print description line
//
//     free string buffer
//     return was_processed flag
...
// function xds_do_channel(subtitle, context):
//     if context is null:
//         return error
//
//     initialize was_processed flag to false
...
//     switch (current xds packet type):
//         case NETWORK_NAME:
//             extract network name from payload
//             log and print network name
//             if name changed from previous:
//                 log network change
//                 update context with new name
//
//         case CALL_LETTERS_AND_CHANNEL:
//             extract call letters from payload
//             log and print call letters
//             if call letters changed:
//                 log change
//                 update context and GUI
//
//         case TSID:
//             extract TSID (Transmission Signal ID) from payload
//             if valid TSID:
//                 print TSID info
//
//     return was_processed flag
...
// function xds_do_private_data(subtitle, context):
//     if context is null:
//         return error
//
//     allocate string buffer (3 bytes per payload byte + 1 for null terminator)
//     if allocation fails:
//         return error
//
//     for each byte in payload (starting from index 2):
//         convert byte to 2-digit hex string and append to buffer
//
//     print the hex dump string to subtitle
//     free the buffer
//     return success
...
//         case TIME_OF_DAY:
//             was_processed = true
//             if payload has enough data:
//                 extract time components:
//                     - minutes (6 bits)
//                     - hours (5 bits)
//                     - date (5 bits)
//                     - month (4 bits)
//                     - reset seconds flag (1 bit)
//                     - day of week (3 bits)
//                     - year (6 bits, offset by 1990)
//                 log formatted time information
//
//         case LOCAL_TIME_ZONE:
//             was_processed = true
//             if payload has enough data:
//                 extract:
//                     - daylight savings flag (1 bit)
//                     - hours offset (5 bits)
